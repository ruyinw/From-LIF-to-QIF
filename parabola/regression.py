...
#     # encoded_inputs = jnp.zeros((num_samples, 32))
    
#     # for i, x in enumerate(xs):
#     #     encoded_inputs = encoded_inputs.at[i].set(encode_input_thermometer(x, 32))

#     # without encoding
#     # normalized = (xs - jnp.min(xs)+1e-6)/(jnp.max(xs)-jnp.min(xs)+1e-6)
#     normalized = (xs - jnp.min(xs))/(jnp.max(xs)-jnp.min(xs))
#     normalized=(1-normalized)*T
#     # normalized = jnp.where(normalized>0.01, jnp.log(normalized/(normalized-0.01))*T, jnp.inf)
...
# #     # Create encoded inputs (spike times for each encoding neuron)
# #     encoded_inputs = jnp.zeros((num_samples, num_neurons_per_input))
...
#     #     t_out_first_ord = times[first_idx]
#     #     t_out_first_pseudo = neuron.t_pseudo(x_end[:, N - Nout + i], input[i], 1, config)
#     #     t_out_first = jnp.where(0 < Kout, t_out_first_ord, t_out_first_pseudo)
            
#     #     # Create a new mask that excludes the first spike
#     #     mask_without_first = mask & (jnp.arange(len(mask)) != first_idx)
#     #     # Kout_remaining = jnp.sum(mask_without_first)
            
#     #     # Find the second spike
#     #     t_out_second_ord = times[jnp.argmax(mask_without_first)]
#     #     t_out_second_pseudo1 = neuron.t_pseudo(x_end[:, N - Nout + i], input[i], 1, config)
#     #     t_out_second_pseudo2 = neuron.t_pseudo(x_end[:, N - Nout + i], input[i], 2, config)
#     #     t_out_second =jnp.select(
#     #         [Kout == 0, Kout == 1, True],
#     #         [t_out_second_pseudo2, t_out_second_pseudo1, t_out_second_ord]
#     #     )
     
#     #     # return jnp.array([t_out_first, t_out_second])
#     #     return jnp.array(t_out_first), jnp.array(t_out_second)

   
#     # t_outs1, t_outs2 = vmap(compute_tout)(jnp.arange(Nout))
#     # # print(t_outs1.shape, t_outs2.shape)
#     # # t_outs = [t_outs1, t_outs2]
#     # t_outs = t_outs2 - t_outs1
...
#     # print(t_out.shape, t_out[0].shape, t_out[1].shape)
#     # loss = ((1/t_out[0]) - target) ** 2
...
# dydx = jax.vmap(fn_dydx, in_axes=(None, None, 0, None))(neuron, p, input, config)
